	...
	for (int i = 0; i < BULLETGAUGE_MAX; i++)
	{
		if (BulletGaugeUse[i])
		{
			...
			{	// 頂点フォーマットの設定
				...
				// ポリゴンの描画
				pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_POLYGON, VertexWk[TEX_NUM001 + i], sizeof(VERTEX_2D));
			}
			...
			{	// 頂点フォーマットの設定
				...
				// ポリゴンの描画
				pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_POLYGON, VertexWk[TEX_NUM003 + i], sizeof(VERTEX_2D));
			}
		}
	}
...
//=============================================================================
// テクスチャ座標の設定
// 引　数：int index(ゲージのアドレス), float val(X軸のゲージ変動率)
// 戻り値：な　し
//=============================================================================
void SetTextureBulletGauge(int index, float val)
{
